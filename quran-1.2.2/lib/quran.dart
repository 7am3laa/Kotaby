...
import 'package:quran/test_quran.dart';
...
int getJuzNumberByPage(int pageNumber) {
  dynamic pageData = getPageData(pageNumber);

  return getJuzNumber(pageData[0]["surah"], pageData[0]["start"]);
}
...
String getSurahNameArabicFull(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surah[surahNumber - 1]['name_arabic'].toString();
}
...
String getVerseTest(int surahNumber, int verseNumber,
    {bool verseEndSymbol = false}) {
  String verse = "";
  for (var i in testQuran) {
...
String getVerseQCFV1(int surahNumber, int verseNumber,
  ...
  for (var i in quranText) {
    if (i['surah_number'] == surahNumber && i['verse_number'] == verseNumber) {
      verse = i['qcfDataV1'].toString(); //print(verse);
      break;
    }
  }
...
List<dynamic> getversesBySurah(int surahNumber) {
  List<int> verses = [];
  for (var i in quranText) {
    if (i['surah_number'] == surahNumber) {
      verses.add(i['verse_number']);
    }
  }
  return verses;
}

List<String> getAudioURLSurah(int surahNumber, reciterIdentifier) {
  int verseNum = 0;
  ...
  if (reciterIdentifier == "ar.parhizgar" ||
    ...
    for (var element in indexs) {
      audioUrls.add(
          "https://cdn.islamic.network/quran/audio/128/$reciterIdentifier/$element.mp3");
    }
    //return "https://cdn.islamic.network/quran/audio/128/$reciterIdentifier/$verseNum.mp3";
    return audioUrls;
  } else {
    for (var element in indexs) {
      audioUrls.add(
          "https://cdn.islamic.network/quran/audio/64/$reciterIdentifier/$element.mp3");
    }
